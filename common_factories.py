...
def addS3Tests(factory, mtrDbPool):
    factory.addStep(
        steps.MasterShellCommand(
            name="Create Bucket",
            command=["mc", "mb", "minio/%(prop:buildername)s-%(prop:buildnumber)s"],
        ...
        steps.MTR(
            ...
            command=[
                ...
                util.Interpolate(
                    r"""
           cd mysql-test &&
           exec perl mysql-test-run.pl --verbose-restart --force --retry=3 --max-save-core=2 --max-save-datadir=10 --mem --parallel=$(expr %(kw:jobs)s \* 2) %(kw:mtr_additional_args)s --suite=s3; fi
           """,
                    mtr_additional_args=util.Property(
                        "mtr_additional_args", default=""
                    ),
    ...
    factory.addStep(
        steps.MasterShellCommand(
            ...
            command=[
                ...
                "rb",
                "--force",
                "minio/%(prop:buildername)s-%(prop:buildnumber)s",
            ],
            doStepIf=lambda props: hasS3(props)
            and props.hasProperty("compile_step_completed"),
        )
