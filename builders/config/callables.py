...
def canStartBuild(
    ...
    for bfw in wfb.worker.workerforbuilders.values(): # iterate through builders assigned to current worker
        if bfw.isBusy(): # isBusy means another builder is running on the current worker so we need to calculate how many jobs were claimed
            other_builder_job_claim = bfw.builder.config.properties["jobs"] # This property (config-time) must exist at Builder level
