...
from configuration.builders.infra.runtime import BuildSequence,CleanupWorkerDir,CleanupDockerResources, FetchContainerImage, TagContainerImage, ContainerCommit,CreateDockerWorkdirs
...
class BaseBuilder:
    ...
    def get_factory(self) -> BuildFactory:
        ...
        for seq in self.build_sequences:
            for step in seq.get_steps():
                if hasattr(step, "run_in_container") and step.run_in_container:
                    if str(step.command.workdir) not in docker_workdirs and not step.command.workdir.is_absolute():
                        docker_workdirs.append(str(step.command.workdir))

                    if not has_docker_enivornment:
                        has_docker_enivornment = True
                        self.prepare_steps.append(CleanupDockerResources(name="previous", config=step.docker_environment))
                        self.cleanup_steps.append(CleanupDockerResources(name="current", config=step.docker_environment))

                    if docker_config != step.docker_environment:
                        docker_config = step.docker_environment
                        self.prepare_steps.append(FetchContainerImage(config=step.docker_environment))
                        ...
                        self.active_steps.append(TagContainerImage(config=step.docker_environment))
                        docker_config = step.docker_environment
                    self.active_steps.append(step.generate())

                    if step.container_commit:
                        self.active_steps.append(ContainerCommit(config=step.docker_environment, step_name=step.name))
                else:
                    self.active_steps.append(step.generate())
        ...
        if docker_workdirs:
            self.prepare_steps.append(CreateDockerWorkdirs(config=docker_config, workdirs=docker_workdirs))
