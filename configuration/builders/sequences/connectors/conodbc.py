            ...
            ShellStep(
                command=ConfigureMariaDBCMake(
                    ...
                    cmake_generator=CMakeGenerator(
                        ...
                        flags=[
                            CMakeOption(CMAKE.BUILD_TYPE, BuildType.RELWITHDEBUG),
                            CMakeOption(OTHER.PACKAGE_PLATFORM_SUFFIX, typ),
        ...
        InContainer(
            ShellStep(
                command=BashCommand(
                    ...
                    cmd='apt install -y unixodbc && sed -i "s/localhost/$SIDECAR_HOST/" odbc.ini && export TEST_SERVER=$SIDECAR_HOST && ./odbc_basic',
                    user="root",
                ),
                env_vars=[
