...
from configuration.steps.commands.compile import CompileMakeCommand, InstallRPMFromProp, MAKE
from configuration.steps.commands.configure import ConfigureMariaDBCMake
from configuration.steps.commands.fetch_file import FindFiles, FetchTarball, FetchCompat
from configuration.steps.commands.packages import CreateRpmRepo, SavePackages
from configuration.steps.commands.mtr import MTRTest, ArchiveLogs
from configuration.steps.base import StepOptions
from configuration.steps.generators.cmake.generator import CMakeGenerator
from configuration.steps.generators.cmake.options import CMAKE, CMakeOption, OTHER, BuildConfig
...
from utils import hasPackagesGenerated, savePackageIfBranchMatch, hasFailed


def rpm_autobake(config, jobs, buildername, rpm_type, arch, artifacts_url, has_compat=False):
    steps = []
    if has_compat:
        steps.append(
            ShellStep(
    ...
    steps.extend([
        ShellStep(FetchTarball(workdir="padding_for_CPACK_RPM_BUILD_SOURCE_DIRS_PREFIX")),
        ShellStep(
        ...
        ShellStep(CompileMakeCommand(
            ...
            workdir="padding_for_CPACK_RPM_BUILD_SOURCE_DIRS_PREFIX")),
        PropFromShellStep(
            command=FindFiles(include="*.rpm", exclude="*.src.rpm", workdir="padding_for_CPACK_RPM_BUILD_SOURCE_DIRS_PREFIX"),
            property="packages",
        ),
        DockerShellStep(
        ...
        ShellStep(
            MTRTest(name="galera", 
                    ...
                    testcase=MTRGenerator(
                            ...
                            suite_collection=TestSuiteCollection([SUITE.WSREP, SUITE.GALERA, SUITE.GALERA_3NODES, SUITE.GALERA_3NODES_SR])
            )
                        ),
        ),
