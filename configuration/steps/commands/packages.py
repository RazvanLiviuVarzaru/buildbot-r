...
set -euo pipefail
apt-get update
apt install -y apt-utils apt-transport-https ca-certificates
...
# Add repo
echo "deb [trusted=yes allow-insecure=yes] $BASE_URL/$ID $SUITE {self.components}" \
...
class SetupRPMRepo(Command):
    def __init__(self, repo_name: str, repo_url: str):
        ...
        super().__init__(
            name=f"Setup RPM repository: {repo_name}",
            workdir=PurePath("."),
            user="root",
        )
