...
get_rpm_info() {
    ...
    while IFS= read -r FILENAME; do
        ...
        if [ "$MODE" == "requires" ]; then
            rpm -q --requires -p "$FILEPATH_CI" 2>/dev/null | sed -e 's/>=.*/>=/' -e 's/\([A-Z0-9._]*\)\([0-9]*bit\)$//' -e '/MariaDB-compat/d' -e '/rpmlib(FileCaps)/d' -e 's/(GLIBC_[^)]*)//' > "$REQUIRES_DIR/CI/$FILENAME.requires"
            rpm -q --requires -p "$FILEPATH_REBUILT" 2>/dev/null | sed -e 's/>=.*/>=/' -e 's/\([A-Z0-9._]*\)\([0-9]*bit\)$//' -e '/MariaDB-compat/d' -e '/rpmlib(FileCaps)/d' -e 's/(GLIBC_[^)]*)//' > "$REQUIRES_DIR/REBUILT/$FILENAME.requires"
        elif [ "$MODE" == "content" ]; then
